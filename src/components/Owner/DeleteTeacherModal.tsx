...
export function DeleteTeacherModal({ isOpen, onClose, teacher, onSuccess }: DeleteTeacherModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in">
      <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 shadow-2xl border border-rose-300 dark:border-rose-900 space-y-6 animate-in zoom-in-95 text-slate-900 dark:text-white">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
          <div className="flex items-center gap-3 text-rose-600 dark:text-rose-400">
            ...
            <div>
              <h3 className="font-black text-lg text-slate-900 dark:text-white">حذف المعلم نهائيًا (Permanent Delete)</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold">{teacher.name} ({teacher.email})</p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3 pt-2">
          <button
            ...
            className="flex-1 py-3.5 bg-rose-600 hover:bg-rose-700 disabled:bg-slate-300 dark:disabled:bg-slate-800 text-white disabled:text-slate-400 font-black text-xs rounded-2xl shadow-lg shadow-rose-600/20 transition active:scale-95 flex items-center justify-center gap-2"
          >
